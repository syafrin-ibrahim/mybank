...
import (
	...
	mockdb "github.com/syafrin-ibrahim/mybank/db/mock"
	db "github.com/syafrin-ibrahim/mybank/db/sqlc"
	"github.com/syafrin-ibrahim/mybank/token"
	...
	testCases := []struct {
	...
	}{
		{
			...
			checkResponse: func(t *testing.T, recorder *httptest.ResponseRecorder) {
				...
				requireBodyMacthAccount(t, recorder.Body, acc)
			},
		},
		{
...
func TestCreateAccount(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			name: "OK",
			body: gin.H{"currency": acc.Currency},
			setupAuth: func(t *testing.T, request *http.Request, tokenMaker token.Maker) {
				addAuthorization(t, request, tokenMaker, authorizationTypeBearer, user.Username, time.Minute)
			},
			buildStubs: func(store *mockdb.MockStore) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			buildStubs: func(store *mockdb.MockStore) {
				...
				store.EXPECT().ListAccounts(gomock.Any(), gomock.Eq(arg)).Times(1).Return(acc, nil)
			},
			checkResponse: func(recorder *httptest.ResponseRecorder) {
				require.Equal(t, http.StatusOK, recorder.Code)
				requireBodyMacthAccounts(t, recorder.Body, acc)
			},
		},
		{
			name: "NoAuthorization",
...
func requireBodyMacthAccount(t *testing.T, body *bytes.Buffer, acc db.Account) {
...
func requireBodyMacthAccounts(t *testing.T, body *bytes.Buffer, acc []db.Account) {
	data, err := ioutil.ReadAll(body)
	require.NoError(t, err)
